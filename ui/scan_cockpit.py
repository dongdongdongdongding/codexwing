...
def render_signal_card_list(rows: List[Dict[str, Any]], *, empty_text: str = "표시할 후보가 없습니다.") -> None:
    ...
    for row in rows:
        ...
        with st.container(border=True):
            ...
            with cols[0]:
                priority_label = str(row.get("execution_priority_label") or "").strip()
                priority_rank = row.get("execution_priority_rank") or row.get("rank") or "-"
                st.caption(f"#{priority_rank}" + (f" · {priority_label}" if priority_label else ""))
...
def render_scan_top_candidates(results_df: Any, bridge_info: Dict[str, Any] | None, market: str) -> None:
    ...
    execution_records = build_execution_priority_records(raw_records, planner_payload, limit=5)
    streams = split_stream_records(raw_records)
    stream_a_records = streams["stream_a"]
    stream_b_records = streams["stream_b"]

    execution_rows = build_signal_display_rows(execution_records, limit=5)
    ...
    st.markdown("### 최우선 실전 Top 5 · Exception Leader + Top 후보 조합")
    st.caption(
        "이 블록이 실제로 먼저 봐야 하는 순서입니다. 검증상 Exception Leader를 최우선으로 올리고, "
        "남는 자리는 플래너 Top/Watch 후보로 채웁니다. 원본 스캔 Top5는 아래 비교용입니다."
    )
    if execution_rows:
        render_signal_card_list(execution_rows, empty_text="실전 우선 후보 없음.")
    else:
        st.info("실전 우선 후보 없음 — 현재 스캔에서 실행 후보가 없습니다.")
    ...
    with st.expander("보조 확인 · 원본 Top5 vs 플래너 후보", expanded=False):
        st.caption(
            "원본 스캔 상위는 Decision Score 기준 참고 목록입니다. 최상단 실전 Top은 "
            "Exception Leader와 플래너 후보를 성과 우선순위로 조합한 목록입니다."
        )
    ...
    if stream_a_rows:
        render_signal_card_list(stream_a_rows, empty_text="Stream A 후보 없음.")
    else:
    ...
    if stream_b_rows:
        render_signal_card_list(stream_b_rows, empty_text="Stream B 후보 없음.")
    else:
