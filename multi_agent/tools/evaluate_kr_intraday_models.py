...
def build_segment() -> tuple[pd.DataFrame, list[str]]:
    ...
    seg["target"] = (pd.to_numeric(seg["return_1d_pct"], errors="coerce") > 0.0).astype(int)
...
def threshold_sweep(prob: np.ndarray, returns: np.ndarray, target: np.ndarray) -> tuple[list[dict], dict | None]:
    rows = []
    for th in [0.25, 0.30, 0.35, 0.40, 0.45, 0.50, 0.55, 0.60, 0.65, 0.70]:
        mask = prob >= th
        picks = int(mask.sum())
        if picks == 0:
    ...
    viable = [r for r in rows if r["picks"] >= 25 and r["avg_return"] is not None]
...
def fit_and_eval(name: str, model, X_train, X_sel, X_rep, y_train, y_sel, y_rep,
    ...
    **고르는 슬라이스와 재는 슬라이스를 나눈다.**
    ...
    # SWING 은 기준선 대비 리프트 +7.6pp 인데 INTRADAY 는 +0.3pp 로 기준선을 그대로
    # 복제한다. 그 차이는 절대 수치로는 안 보이고 기준선을 실어야 보인다(규율 16).
    ...
    payload = {
        ...
        "auc": auc,
        "segment": "phase25_kr_intraday_benchmark",
        "return_col": "return_1d_pct",
        "positive_threshold": 0.0,
        "recommended_probability_threshold": (best or {}).get("threshold", 0.5),
        "description": f"KR intraday benchmark candidate ({name}) trained on realized 1D > 0%.",
...
def main():
    ...
    returns = pd.to_numeric(seg["return_1d_pct"], errors="coerce")
    ...
    json_path = REPORT_DIR / "kr_intraday_model_benchmark.json"
    md_path = REPORT_DIR / "kr_intraday_model_benchmark.md"
    payload = {
    ...
    lines = ["# KR Intraday Model Benchmark", ""]
    lines.append(f"- rows: `{len(seg)}`")
    lines.append(f"- positives(1D > 0%): `{int(y.sum())}`")
    lines.append("")
