...
def build_signal_display_rows(rows: List[Dict[str, Any]], limit: int | None = None) -> List[Dict[str, Any]]:
    ...
    Card '검증 목표터치 승률' = admission/segment historical OOS
    target-touch win rate. raw model score (phase25_prob, prob_clean, ml_prob)는
...
# Card '검증 목표터치 승률' = segment/admission historical OOS target-touch rate.
TOP_N_CARD_COLUMNS = (
...
def build_top_candidate_compact_view(planner_payload: Dict[str, Any], limit: int = 5) -> Dict[str, Any]:
    ...
    카드 검증 목표터치 승률('Accuracy') = segment/admission historical OOS target-touch rate.
