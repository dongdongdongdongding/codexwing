...
RETIRED_LANES: Dict[str, str] = {
    ...
    # 🔴 2026-09-05 철회 — **조정관 오판이었다.**
    # 2026-09-03 에 이 레인을 「서빙 모델이 2026-05-08 학습, auc 0.478」을 근거로 정지시켰는데,
    # **그 모델은 이 레인의 것이 아니다.** `phase25_kr_intraday_xgboost.pkl` 은 `quant_analysis`
    # 스코어링 경로의 tilt 모델이고, 이 레인은 자기 전용 번들
    # `models/kr_intraday_3d_t5/kosdaq_liq30_1500_lgbm_isotonic_vwapguard.pkl` 로 돈다 —
    # `train_kosdaq_1500_bundle.py` 가 **일일 재학습**한다(run_daily_ops.sh 배선, 실측 2026-09-05 재학습,
    # 학습행 29,763 · 자기 장중 피처 · 보정확률 0.7 컷). 낡지 않았다.
    # 그리고 이 레인은 전진 원장에서 **유일하게 95%CI 하한이 0 을 넘는 레인**이다
    # (n=8 · net EV +7.06% · CI [+0.23,+13.90] · 승률 87.5%) — n 이 작아 단정할 수 없지만
    # **정지의 근거가 아니라 정반대 방향의 증거다.**
    # 교훈: 「서빙 모델」을 말하기 전에 **그 레인의 생산자가 무엇을 로드하는지** 확인해야 한다.
}
