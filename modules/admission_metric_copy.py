...
METRIC_HELP: Dict[str, str] = {
    ...
    "candidate_pass_prob_5d": (
        "현재 후보의 피처를 Admission 모델에 넣어 계산한 후보별 5거래일 내 목표 고가 터치 확률입니다. "
        "운영 기준을 넘으면 Admission 통과, 못 넘으면 Near Miss로 표시됩니다."
    ),
    "candidate_top_prob_5d": (
        "이번 스캔 후보 중 Admission 모델이 가장 높게 평가한 5거래일 내 목표 고가 터치 확률입니다. "
    ...
    "validation_avg_return_5d": (
        "후보별 확정 수익이 아니라 현재 Admission 모델 선택규칙의 과거 검증 표본에서 나온 평균 5거래일 내 최고 고가 상승률입니다."
    ),
    "validation_worst_return_5d": (
        "후보별 확정 손익이 아니라 현재 Admission 모델 선택규칙의 과거 검증 표본에서 가장 낮았던 5거래일 내 최고 고가 상승률입니다."
    ),
    "model_rank": "이번 스캔 후보를 Admission 모델 확률순으로 정렬한 순위입니다.",
    "candidate_model_score_5d": (
        "현재 Admission 경로에서는 후보 목표터치 확률과 같은 값입니다. 과거 보고서 호환을 위해 별도 점수 필드로도 저장합니다."
    ),
    "stop_first_risk_5d": (
