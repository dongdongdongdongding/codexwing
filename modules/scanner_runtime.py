...
def format_hourly_signal_message(signal_data: Dict[str, Any]) -> str:
    """Render legacy Telegram markdown body for one top signal."""
    s = signal_data
...
🎯 **Strategy: 1-2 Day Swing (Limit Buy @ -2%)**
• Entry (Limit): {currency}{float(s.get('entry', 0) or 0):,.2f}
• Target (+3.5%): {currency}{float(s.get('target', 0) or 0):,.2f}
• Stop Loss (-3%): {currency}{float(s.get('stop_loss', 0) or 0):,.2f}
